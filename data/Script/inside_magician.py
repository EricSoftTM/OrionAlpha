...
self.say("#bScript: inside_magician#k\r\n\r\n#rSorry, I am not coded yet.#k")
...
if (self.UserGetJob() == 200 && self.UserGetLevel() >= 30) {
	if (self.InventoryGetItemCount(4031013) >= 30) {
		...
		let ret = self.InventoryExchange(0, 4031013, -nBlack, 4031009, -1, 4031012, 1);
		//Hummm... verifique se voc?tem 30 itens do tipo #t4031013#, a carta de #b#p1032001##k e um slot vazio no seu invent?io de Etc.
		if (!ret) self.say("Please, check whether or not you have room in your Etc inventory.");
		else self.RegisterTransferField(101020000, "");
	} else {
