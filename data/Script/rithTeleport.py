...
let nSel = self.askMenu("It's understable that you may be confused about this place if this is your first time around. If you got any questions about this place, fire away. #b\r\n#L0#What kind of towns are here in Victoria Island?#l\r\n#L1#Please take me somewhere else.#l#k");
if (nSel == 0) {
	nSel = self.askMenu("There are 6 big towns here in Victoria Island. Which of those do you want to know more of?\r\n#b#L0##m104000000##l\r\n#L1##m102000000##l\r\n#L2##m101000000##l\r\n#L3##m100000000##l\r\n#L4##m103000000##l\r\n#L5##m105040300##l");
	if (nSel == 0) {
	...
	menu = (self.UserIsBeginner() ? "There's a special 90% discount for all beginners. Alright, where would you want to go?#b" : "Oh you aren't a beginner, huh? Then I'm afraid I may have to charge you full price. Where would you like to go?#b");
	for (let i = 0; i < towns.length; menu += "\r\n#L" + i + "##m" + towns[i][0] + "# (" + towns[i][1] * val + " mesos)#l", i++);
	let sel = self.askMenu(menu);
	if (sel >= 0 && sel < towns.length) {
		goTown(towns[sel][0], towns[sel][1] * val);
	}
}

function goTown(mapNum, fee) {
	let nRet = self.askYesNo("You don't have anything else to do here, right? Would you like to go to #b#m" + mapNum + "##k? It will cost #b" + fee + " mesos#k.");
	if (nRet == 1) {
